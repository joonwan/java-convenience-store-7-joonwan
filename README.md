...
- [ ] 프로모션 적용 여부를 확인한다.
- [ ] 프로모션이 존재하지 않거나 해당 날짜가 아닌 경우 `DEFAULT` 로 타입을 설정한다.

- [ ] 주문 상품의 주문 수량이 프로모션 재고 내에서 해결 가능한지 확인한다.
    - [ ] 주문 수량이 프로모션 재고보다 많을 경우 주문 상품의 타입을 `PARTIAL_PROMOTION` 로 설정한다.

- [ ] 추가 증정이 가능한지 확인한다.
    - [ ] 주문 수량 - (증정된 상품수량 + 프로모션 차감 상품수량) 이 프로모션의 buy 랑 동일 할 경우 해당 주문 상품의 타입을 `ADDITIONAL_PROMOTION` 로 설정한다.
    - [ ] 아닐 경우 `DEFAULT` 로 설정한다.
