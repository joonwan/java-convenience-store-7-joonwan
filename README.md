...
- [ ] 주문 수량이 정상적인지 확인한다.
  -  [ ] 주문수량이 숫자가 아닐 경우 예외를 발생시킨다.
  -  [ ] 주문 수량이 양수가 아닐 경우 예외를 발생시킨다.
  -  [ ] 주문 수량이 재고 수를 초과할 경우 예외를 발생 시킨다.
- [ ] 각 주문 상품에 프로모션 적용 가능 여부를 확인한다.
  - [ ] 주문 상품에 프로모션 적용 가능할 경우
    - [ ] 프로모션 재고 증정 가능 할 경우 해당 수량을 주문 상품에 업데이트 한다.
    - [ ] 주문 수량이 프로모션 재고를 초과할 경우 프로모션 할인을 받지 못하는 재고 수량을 주문 상품에 업데이트 한다.
- [ ] 예외가 발생할 경우 `[ERROR]` 로 시작하는 오류 메시지와 함께 상황에 맞는 안내문구를 출력한다. 이후 재입력을 받는다.
