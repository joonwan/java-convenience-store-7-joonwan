...
public class InputView {
    ...
    public static String confirmReceivePromotion(OrderProductStatus orderProductStatus) {
        ...
        int count = orderProductStatus.getAdditionalReceiveCount();

        String message = String.format("현재 %s은(는) %d개를 무료로 더 받을 수 있습니다. 추가하시겠습니까? (Y/N)", name, count);
    ...
    public static String confirmPartialPromotion(OrderProductStatus orderProductStatus) {
        ...
        String message = String.format("현재 %s %d개는 프로모션 할인이 적용되지 않습니다. 그래도 구매하시겠습니까? (Y/N)", name, count);
        System.out.println(message);
        return Console.readLine();
    }
}
